...
impl BrawlClient {
	...
	pub fn get_brawlers(&self) -> structs::BrawlerList {
		let res = self.request("https://api.brawlstars.com/v1/brawlers");

		let v: structs::BrawlerList = serde_json::from_str(&res)
			.expect("Function has panicked");

		return v
	}

	pub fn get_player(&self, tag: &str) -> structs::Player {
		// Obtaining the url
		...
		// Returning the correct struct
		let v: serde_json::Value = serde_json::from_str(&res)
			.expect("Function has panicked");

		return structs::Player::new(v)
	}
}